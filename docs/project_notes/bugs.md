...
- **Balance pass (2026-05-31, done):** fixture dodge/parry SMR are no longer uniform — each combatant's SMR is linearly mapped from its own dodge/parry SR onto the Excel `Defense Simulations` SMR bands (dodge 0.80–0.90, parry 0.90–1.00), per-file min→max, so better defenders mitigate more (honors the sheet's rank coupling of SR↔SMR). Top parriers (Kade, Shadow Rogue, Warrior, Bog Shaman) reach parry SMR 1.0 = full negation, accepted as intended at the band ceiling.
- **Still open (ADR-054 Layer B, feature work not a bug):** per-reaction rank/XP progression that *derives* all six dodge/parry rates from a reaction rank via the `Defense Simulations` table (decoded in ADR-054), replacing today's static fixture values.
