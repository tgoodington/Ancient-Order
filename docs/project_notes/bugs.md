...
- **Status:** Corrected model documented in **ADR-054** (2026-05-31). The intended design: all three reactions carry SMR, successful dodge/parry apply `(1 − SMR) × ActionPower` (not 0), rates derive from per-reaction **rank/progression**, and a counter is a *new attack subject to a new reaction* (not a forced parry loop). Implementation scoped as Layer A (in-combat SMR + counter rework) and Layer B (reaction progression) — to be planned via `/intuition-plan`. The Excel Dodge-success base (`VLOOKUP(…$A$31:$D$36,4)` = "Order Speed") is a confirmed spreadsheet bug; intended formula mirrors Parry's `(1 − dodgeSMR) × ActionPower`. Current full-negation model remains in place until that pass lands.
