...
- **Prevention:** A compute helper that is exported but never called from the resolution path is a red flag (same lesson as the result-discard bug). When a subsystem (elemental paths) has no observable effect, trace whether its outputs are actually consumed downstream — not just produced. Balance note recorded in ADR-053: path buffs accumulate unbounded (clamped at 1.0); revisit with a cap/decay if needed.
